...
var restClient = require('request');
 ...
 const welcomeOutput = "Welcome to MLC Life Insurance. <break time='250ms'/> You can get an indicative quote  by saying  <break time='250ms'/> get quote  <break time='250ms'/>or talk to an adviser by saying  <break time='250ms'/> find me an adviser. <break time='250ms'/> I have sent the details to your Alexa App for further reference. ";
...
const handlers = {
  ...
  //    this.emit(':tell',"Your call back option is set for" + slots['time'].value);

  let speech = new Speech();
   speech.say("Your call back  ")
          .pause('150ms')
        //  .sayAs({
      //    word: slots['phonenumber'].value,
      //    interpret: "telephone"
      //    })
         .say("is set for")
         .pause('150ms')
         .sayAs({
         word: slots['time'].value,
         interpret: "time"
       });
 ...
 const nexmo = new Nexmo({
   apiKey: 'ff3c97f7',
   apiSecret: 'DMoV7QYHtfAxixZ5'
 })

 const from = 'MLC Life'
 const to = 61412715607
 //const text = 'Please call back on'+ slots['phonenumber'].value + 'at' + slots['time'].value;
 const text = 'Please call back on';
        ...
        find( {"suburb": slots["suburb"].value } ).toArray(function(err,results){
                ...
                let speech = new Speech();
                 speech.say('Adviser, ' + results[0].name  + ' is available at')
                       ...
                       .pause('150ms')
                       .say('I have sent the complete list of advisers to your Alexa App.')
                       .pause('150ms')
                       .say('If you are interested for call back, please provide your number and preferred time');
                       let speechOutput = speech.ssml(true);
                      handler1.emit(':ask', speechOutput);
